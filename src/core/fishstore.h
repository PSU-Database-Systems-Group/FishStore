...
// Thread-local parser context
    ...
    public:
        ...
        FishStore(uint64_t table_size, uint64_t log_size, const std::string &filename)
                : min_table_size_{table_size}, disk{filename, epoch_}, hlog{log_size, epoch_, disk, disk.log()},
                  system_state_{Action::None, Phase::REST, 1}, num_pending_ios{0}, system_parser_no_{0} {
        ...
        while (A::HasNext(parser_ctx().parser)) {
            ...
            for (auto general_psf_id: parser_state.ptr_general_psf) {
                ...
                if (psf.lib_id == LIB_EZ_PSF) {
                    ezpsf::StringRef string_ref{nullptr, 0};
                    bool has_value = psf.ez_eval(&psf_args, &string_ref);

                    // if it is an int64, we need to turn it to a pointer
                    // and length of pointer, as it won't inline (8 bytes)
                    if (psf.ret_type == ezpsf::DataType::INT64_T) {
                        auto val = new int64_t(*(int64_t *) &string_ref);
                        res = NullableStringRef{
                                .is_null=!has_value,
                                .need_free=true,
                                .size=sizeof(int64_t),
                                .payload = (char *) val
                        };
                    } else if (psf.ret_type == ezpsf::DataType::STR_T) {
                        res = NullableStringRef{
                                .is_null=!has_value,
                                .need_free=false,
                                .size = static_cast<uint32_t>(string_ref.size),
                                .payload = string_ref.data
                        };
                    } else {
                        LOG("Unsupported return type (EzPsf needs to be STR_T or INT64_T). Found: " +
                            ezpsf::getName(psf.ret_type));
                    }
                } else if (psf.lib_id == LIB_PROJECTION)
                    res = projection<A>(psf_args);
                else
...
tot_cpu_time += std::chrono::duration<double>(t_end - t_start).count();
    ...
    inline OperationStatus FishStore<D, A>::InternalInsert(C &pending_context) {
        ...
        new(record) record_t{RecordInfo{
                ...
                static_cast<uint32_t>(kpts.size()), pending_context.payload_size(),
                pending_context.optional_size()}};
...
return OperationStatus::SUCCESS;
    ...
    template<class D, class A>
    inline Address FishStore<D, A>::BlockAllocate(uint32_t
                                                  record_size) {
        uint32_t page;
        Address retval = hlog.Allocate(record_size, page);
        while (retval < hlog.read_only_address.
...
// Scan IO callback based on adaptive IO callback.
            ...
            if (context->address > pending_context->end_addr) {
                ...
                if (record->size() > available_bytes) {
                    ...
                    while (current_address >= pending_context->start_addr &&
                        ...
                        kpt_now = reinterpret_cast<KeyPointer *>(io_base + current_address.scan_offset(
                                current_scan_offset_bit));
                        if (current_address.scan_offset(current_scan_offset_bit) <
                            record_t::ptr_offset(kpt_now->ptr_offset))
                            break;
    ...
    Status FishStore<D, A>::ReadCprContexts(const Guid &token, const Guid *guids) {
        for (size_t idx = 0; idx < Thread::kMaxNumThreads; ++idx) {
            ...
            auto result = checkpoint_.continue_tokens.insert(
                    {context.guid, std::make_pair(context.serial_num, context.offset)});
    ...
    bool FishStore<D, A>::Checkpoint(const std::function<void(Status)> &index_persistence_callback,
                                     const std::function<void(Status, uint64_t,
                                                              uint32_t)> &hybrid_log_persistence_callback,
                                     Guid &token) {
#ifdef _TIMER
...
#undef BREAK_NOT_OK
    ...
    PsfLookup FishStore<D, A>::MakeEzPsf(const std::string &ez_psf_str) {
        ...
        // extract the fields
        for (const auto &field: psf_info.fields) {
            uint16_t field_id;
            auto it = field_lookup_map.find(field);
            if (it != field_lookup_map.end()) {
                field_id = it->second;
        ...
        if (ezpsf::canInline(psf_info.type)) {
            InlinePSF<A> psf;
            psf.fields = field_ids;
            psf.func_name = psf_info.name;
            psf.lib_id = LIB_EZ_PSF;
            psf.ez_eval = psf_info.psf;

            uint64_t inline_psf_id = inline_psf_map.size();
            if (inline_psf_id > (1LL << 32) - 1) {
                printf("Maximum # inline PSF exceeded...\n");
                return {static_cast<uint32_t>(-1), PsfType::INLINE};
            }
            ...
            psf.lib_id = LIB_EZ_PSF;
            psf.ez_eval = psf_info.psf;
            psf.ret_type = psf_info.type;

            uint64_t general_psf_id = general_psf_map.size();
            if (general_psf_id > (1LL << 14) - 1) {
                printf("Maximum # general PSF exceeded...\n");
                return {static_cast<uint32_t>(-1), PsfType::GENERAL};
